  ...
  <UPage>
    <div>
      <div class="text-justify p-5">
        ...
        <p>This is my portfolio site where I showcase and detail the programming projects that I work on in my spare
          time.</p>
        <p>My overall goal is to learn a variety of programming languages covering a variety of topics.</p>
        <p>Each time I'll undertake learning a new programming language, I'll endeavour to develop a publicly viewable
          project in my GitHub profile, as well as providing some documentation on this website.</p>
      </div>
      <UPageColumns class="">
        ...
        <ULink :to="config.public.TWITTER_PROFILE" :external="true">
          <UPageCard title="Twitter/X" description="A link to twitter/x profile" icon="i-simple-icons-x" />
        </ULink>
      </UPageColumns>
    </div>
    ...
    <UPage>
      ...
      <div class="p-4">Checkout a collection of my projects below, alternatively, checkout the Projects page for all of
        my projects</div>

      <UPageGrid>
        <UPageCard v-for="(project, index) in projects" :key="index" v-bind="project">
        </UPageCard>
      </UPageGrid>
    </UPage>
...
const { data: posts } = await useAsyncData("blogs", () => { return queryCollection("blog").order("date", "DESC").limit(3).all() }
);
// if (!posts.value) {
//   throw createError({
//     statusCode: 404,
